...
def get_output_filename(language, model_name):
    result_folder = f"results_task3a_ollama_{model_name.replace(':', '_')}"
    os.makedirs(result_folder, exist_ok=True)
    base_name = f"{language}_task3a"
    version = 1
...
def detect_language(filename):
    for lang in ["UK", "IE", "DE", "SR"]:
        if filename.endswith(f"{lang}.json"):
            return lang
    return "UNKNOWN"

def extract_numeric_answer(text):
    """Extract the first number from 0 to 8 (or 10) from the response"""
    match = re.search(r"\b([0-8]|10)\b", text)
    return match.group(1) if match else "ERR"
    

def run_task3a_for_model(model_name):
