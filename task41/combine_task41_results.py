# combine_task41_results.py - include ollama/online folders + global upload CSV
import os
import csv
import re
...
# Match per-test files like: results_DE_task41_01.csv or DE_task41_01.csv
pattern = re.compile(r"^(?:results_)?(EN|DE|SR)_task41_(\d+)\.csv$")
...
def derive_model_name(folder_name: str) -> str:
    """
    Examples:
      results_task41_online_openai_gpt-4o    -> openai_gpt-4o
      results_task41_ollama_gemma3_27b       -> gemma3_27b
...
# NEW: only scan Task 41 result folders (includes ollama/online/openai variants)
result_folders = [
    f for f in os.listdir(base_path)
    if os.path.isdir(f) and f.startswith("results_task41_")
]

# Global upload target
UPLOAD_DIR = os.path.join(base_path, "_upload")
os.makedirs(UPLOAD_DIR, exist_ok=True)
global_out = os.path.join(UPLOAD_DIR, "results_task41.csv")
...
for results_folder in result_folders:
    ...
    for lang, files in files_by_lang.items():
        files.sort()
        output_file = os.path.join(results_folder, f"combined_{lang}_task41.csv")
    ...
    if files_by_lang:
        print(f"✅ Combined task41 files created in: {results_folder}")
    else:
        print(f"⚠️ No matching task41 files found in: {results_folder}")

# Write the single _upload\results_task41.csv with all rows
