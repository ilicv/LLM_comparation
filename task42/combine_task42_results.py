# combine_task42_results.py - include ollama/online folders + global upload CSV
import os
import csv
import re
...
# Match per-test files like: results_DE_task42_01.csv or DE_task42_01.csv
pattern = re.compile(r"^(?:results_)?(EN|DE|SR)_task42_(\d+)\.csv$")
...
def derive_model_name(folder_name: str) -> str:
    """
    Examples:
      results_task42_online_openai_gpt-4o    -> openai_gpt-4o
      results_task42_ollama_gemma3_27b       -> gemma3_27b
...
# NEW: only scan Task 42 result folders (includes ollama/online/openai variants)
result_folders = [
    f for f in os.listdir(base_path)
    if os.path.isdir(f) and f.startswith("results_task42_")
]

# Global upload target
UPLOAD_DIR = os.path.join(base_path, "_upload")
os.makedirs(UPLOAD_DIR, exist_ok=True)
global_out = os.path.join(UPLOAD_DIR, "results_task42.csv")
...
for results_folder in result_folders:
    ...
    for lang, files in files_by_lang.items():
        files.sort()
        output_file = os.path.join(results_folder, f"combined_{lang}_task42.csv")
    ...
    if files_by_lang:
        print(f"✅ Combined task42 files created in: {results_folder}")
    else:
        print(f"⚠️ No matching task42 files found in: {results_folder}")

# Write the single _upload\results_task42.csv with all rows
