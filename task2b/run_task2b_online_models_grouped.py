# run_task2b_online_models_grouped.py
# Processes attached prompts_* JSON files (DE, SR, IE, UK).
...
# ---- Config ------------------------------------------------------------------
RESULTS_BASE = "results_task2b_online"
...
# The four attached prompt files we need to process:
PROMPT_FILES = [
    "prompts_task2b\\prompts_DE.json",
    "prompts_task2b\\prompts_SR.json",
    "prompts_task2b\\prompts_IE.json",
    "prompts_task2b\\prompts_UK.json",
]

PROVIDERS = {
    "openai": {
        "models": [OPENAI_MODEL],
        "call": lambda sys_msg, uq, model: oo.chat_with_openai(oo.client, sys_msg, uq, model=model),
    },
    "mistral": {
...
# ---- Helpers -----------------------------------------------------------------
def extract_grades_multi_0to10_77_88(text: str, n_expected: int) -> List[str]:
    """
    Extract first n_expected tokens from the set {0..10, 77, 88}.
    Accepts spaces/commas/newlines. Pads with 'ERR2' if fewer returned.
    """
    body = text.split("</think>")[-1] if "</think>" in text else text
    # Order matters: match '10' before single digits; include 77 and 88
    nums = re.findall(r"\b(?:10|[0-9]|77|88)\b", body)
...
def build_combined_prompt(sys_msg: str, questions: List[str]) -> Tuple[str, str]:
    ...
    (Task2b uses 0-10 with 77/88 special options.)
    """
    n = len(questions)
    sys_multi = (
        sys_msg.strip()
        + f"\n\nINSTRUCTION: Answer ALL questions below in ONE line as exactly {n} numbers "
          "(each must be one of 0..10, 77, or 88) separated by single spaces, in the same order. "
...
for provider, cfg in PROVIDERS.items():
    ...
    for model_name in cfg["models"]:
        print(f"\n?? Running Task2b on provider: {provider}  model: {model_name}")
