# run_task3b_online_models_grouped.py
...
# ---- Config ------------------------------------------------------------------
RESULTS_BASE = "results_task3b_online"
...
PROMPT_DIR = "prompts_task3b"
PROMPT_FILES = [
    os.path.join(PROMPT_DIR, "prompts_IE.json"),
    os.path.join(PROMPT_DIR, "prompts_UK.json"),
...
# ---- Helpers -----------------------------------------------------------------
def extract_grades_multi_0to10(text: str, n_expected: int) -> List[str]:
    """
    Extract the first n_expected standalone numbers in the set {0..10}.
    Accepts spaces/commas/newlines; pads with 'ERR2' if fewer returned.
    """
    body = text.split("</think>")[-1] if "</think>" in text else text
    nums = re.findall(r"\b(?:10|[0-9])\b", body)
...
def build_combined_prompt(sys_msg: str, questions: List[str]) -> Tuple[str, str]:
    ...
    (Task3b uses a 0-10 scale.)
    """
    n = len(questions)
    sys_multi = (
        sys_msg.strip()
        + f"\n\nINSTRUCTION: Answer ALL questions below in ONE line as exactly {n} numbers "
          "(each 0-10) separated by single spaces, in the same order. "
...
for provider, cfg in PROVIDERS.items():
    ...
    for model_name in cfg["models"]:
        print(f"\n?? Running Task3 on provider: {provider}  model: {model_name}")
        ...
        for prompt_file in PROMPT_FILES:
            ...
            prefix = f"results_{stem}_task3b_"
            ...
            try:
                ...
                nums = extract_grades_multi_0to10(resp, len(questions))
