# run_task2a_online_models_grouped.py
...
# ---- Config ------------------------------------------------------------------
RESULTS_BASE = "results_task2a_online"
...
PROMPT_FILES = [
    "prompts_task2a\\prompts_DE.json",
    "prompts_task2a\\prompts_SR.json",
    "prompts_task2a\\prompts_IE.json",
    "prompts_task2a\\prompts_UK.json",
]
...
# ---- Helpers -----------------------------------------------------------------
def extract_grades_multi_0to6(text: str, n_expected: int) -> List[str]:
    """
    Extract the first n_expected standalone digits 0-6 from the response.
    Accepts spaces/commas/newlines; pads with 'ERR2' if fewer returned.
    """
    body = text.split("</think>")[-1] if "</think>" in text else text
    nums = re.findall(r"\b[0-6]\b", body)
...
def build_combined_prompt(sys_msg: str, questions: List[str]) -> Tuple[str, str]:
    ...
    (Task2a uses a 0-6 scale.)
    """
    n = len(questions)
    sys_multi = (
        sys_msg.strip()
        + f"\n\nINSTRUCTION: Answer ALL questions below in ONE line as exactly {n} numbers "
          "(each 0-6) separated by single spaces, in the same order. "
...
for provider, cfg in PROVIDERS.items():
    ...
    for model_name in cfg["models"]:
        print(f"\n?? Running Task2a on provider: {provider}  model: {model_name}")
        ...
        for prompt_file in PROMPT_FILES:
            ...
            prefix = f"results_{stem}_task2a_"
            ...
            try:
                ...
                nums = extract_grades_multi_0to6(resp, len(questions))
